...
const StockData = () => {
  ...
  const fetchStockData = async () => {
    try {
      const response = await fetch(`http://localhost:8000/api/stock-data/?ticker=${ticker}&exchange=${exchange}&investmentType=${investmentType}&start_date=${startDate}&end_date=${endDate}`);
      if (!response.ok) {
